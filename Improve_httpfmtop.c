//g++ Improve_httpfmtop.c -o newhttpfmtop blake3.h libblake3.a MurmurHash3.o siphash.c -lcityhash -lpcap -lmaxminddb -lpthread -lmicrohttpd -lcjson -ljansson -ljson-c
...
int DATA_SIZE_THRESHOLD = 100000000;
int FLOW_COUNT_THRESHOLD = 200000;
int SRC_IP_THRESHOLD = 300000;
int DST_IP_THRESHOLD = 300000;
int SRC_PORT_THRESHOLD = 300000;
int DST_PORT_THRESHOLD = 300000;
int PROTOCOL_THRESHOLD = 500000;

volatile int intervalSeconds = 1; 

#define TRAFFIC_THRESHOLD 104857600
volatile uint64_t totalTrafficSize = 0;
volatile uint64_t lastTotalTrafficSize = 0;
pcap_t *handle = NULL;
...
typedef struct TimeNode {
    char timestamp[50];
    uint64_t trafficSizeDifference;
    FlowNode* flows;
...
Alert* alertsHead = NULL;
pthread_mutex_t alertsMutex = PTHREAD_MUTEX_INITIALIZER;
...
const uint8_t siphashKey[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
void hashWithBlake3(const void* input, size_t length, uint32_t* hashOutput) {
    ...
    blake3_hasher_finalize(&hasher, output, sizeof(uint32_t)); // 只取哈希值的前4个字节

    memcpy(hashOutput, output, sizeof(uint32_t));
}
...
void addAlert(const char* timestamp, const char* alertType, const char* message) {
    Alert* newAlert = (Alert*)malloc(sizeof(Alert));
    if (newAlert == NULL) {
        fprintf(stderr, "Failed to allocate memory for new alert.\n");
        return;
        }
    strncpy(newAlert->timestamp, timestamp, sizeof(newAlert->timestamp) - 1);
    strncpy(newAlert->alertType, alertType, sizeof(newAlert->alertType) - 1);
    strncpy(newAlert->message, message, sizeof(newAlert->message) - 1);
    newAlert->next = NULL;

    pthread_mutex_lock(&alertsMutex);
    newAlert->next = alertsHead;
    alertsHead = newAlert;
    pthread_mutex_unlock(&alertsMutex);
}
...
void logAlert(DataNode* dataNode, FlowNode* flowNode, const char* timestamp, const char* alertType, int value, const char* srcCountry, const char* dstCountry) {
    ...
    if (strcmp(alertType, "DataSize") == 0) {
        snprintf(message, sizeof(message), "High data flow detected at TimeNode: %s, Source: %s:%d (%s), Destination: %s:%d (%s), Protocol: %s, Data Size: %d bytes",
                timestamp, flowNode->src_IP, flowNode->src_port, srcCountry, flowNode->dst_IP, flowNode->dst_port, dstCountry, flowNode->protocol, value);
        fprintf(logFile, "[%s] %s myApplication: %s\n", formattedTime, hostName, message);
    } else if (strcmp(alertType, "FlowCount") == 0) {
        snprintf(message, sizeof(message), "High flow count detected at TimeNode: %s, Source: %s:%d (%s), Destination: %s:%d (%s), Protocol: %s, Flow Count: %d",
                timestamp, flowNode->src_IP, flowNode->src_port, srcCountry, flowNode->dst_IP, flowNode->dst_port, dstCountry, flowNode->protocol, value);
        fprintf(logFile, "[%s] %s myApplication: %s\n", formattedTime, hostName, message);
    }

    addAlert(formattedTime, alertType, message); // Ensure meaningful message is added here
    fclose(logFile);
}
...
TimeNode* queryTimestampHashTable(const char* timestamp) {
    uint8_t hashOutput[8]; // SipHash output is 64 bits
    siphash(timestamp, strlen(timestamp), siphashKey, hashOutput, sizeof(hashOutput));
    uint32_t hashValue = *((uint32_t*)hashOutput);
...
void packetHandler(u_char *userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    */
    ...
    char protocolStr[10]; 
    uint64_t srcIPHash, dstIPHash;
    uint32_t srcPortHash, dstPortHash, protocolHash;
    uint32_t protocolHashValue, srcPortHashValue, dstPortHashValue;
    uint8_t hashOutput[8];
    ...
    srcIPHash = CityHash64(srcIP, strlen(srcIP));
    dstIPHash = CityHash64(dstIP, strlen(dstIP));
    hashWithBlake3(&srcPort, sizeof(srcPort), &srcPortHash);
    hashWithBlake3(&dstPort, sizeof(dstPort), &dstPortHash);
    MurmurHash3_x64_128(&ipHeader->ip_p, sizeof(ipHeader->ip_p), HASH_SEED, &protocolHash);
    //srcPortHash = CityHash64((char *)&srcPort, sizeof(srcPort));
    //dstPortHash = CityHash64((char *)&dstPort, sizeof(dstPort));
    //protocolHash = CityHash64((char *)&ipHeader->ip_p, sizeof(ipHeader->ip_p));

    insertIntoART(&artRoot, (uint8_t *)srcIP, strlen(srcIP), srcIPHash, 0);
    insertIntoART(&artRoot, (uint8_t *)dstIP, strlen(dstIP), dstIPHash, 0);
    insertIntoPortsHashTable(srcPort, srcPortHashValue);
    insertIntoPortsHashTable(dstPort, dstPortHashValue);
    //insertIntoPortsHashTable(srcPort, srcPortHash);
    //insertIntoPortsHashTable(dstPort, dstPortHash);
    //insertIntoProtocolsHashTable(ipHeader->ip_p, protocolHash);
    insertIntoProtocolsHashTable(ipHeader->ip_p, protocolHashValue);
...
bool ipMatchesPattern(const char* ip, const char* pattern) {
    const char* ipSegment = ip;
    const char* patternSegment = pattern;
    char ipPart[16];
    char patternPart[16];

    while (*ipSegment && *patternSegment) {
        // Extract IP segment
        const char* ipDot = strchr(ipSegment, '.');
        size_t ipLen = ipDot ? ipDot - ipSegment : strlen(ipSegment);
        strncpy(ipPart, ipSegment, ipLen);
        ipPart[ipLen] = '\0';

        // Extract pattern segment
        const char* patternDot = strchr(patternSegment, '.');
        size_t patternLen = patternDot ? patternDot - patternSegment : strlen(patternSegment);
        strncpy(patternPart, patternSegment, patternLen);
        patternPart[patternLen] = '\0';

        // If pattern segment is not '*', it must match the IP segment
        if (strcmp(patternPart, "*") != 0 && strcmp(ipPart, patternPart) != 0) {
            return false;
        }

        // Move to the next segment
        ipSegment += ipLen + (ipDot ? 1 : 0);
        patternSegment += patternLen + (patternDot ? 1 : 0);
    }

    // Both IP and pattern must be fully parsed
    return *ipSegment == '\0' && *patternSegment == '\0';
}
...
void queryFlow(FlowNode* headFlow, const char* src_IP, const char* src_port_str, const char* dst_IP, const char* dst_port_str, const char* protocol, time_t startTime, time_t endTime, bool useStartTime, bool useEndTime) {
    ...
    // Use useStartTime and useEndTime parameters to decide whether to query TimeNode
    if (useStartTime || useEndTime) {
        if (useStartTime && startTime != firstPacketTime) {
            char startTimeStr[50];
            strftime(startTimeStr, sizeof(startTimeStr), "%Y-%m-%d-%H:%M:%S", localtime(&startTime));
            TimeNode* startTimeNode = queryTimestampHashTable(startTimeStr);
            if (!startTimeNode) {
                printf("Start timestamp not found in the data structure.\n");
                return;
            }
        }

        if (useEndTime && endTime != lastPacketTime) {
            char endTimeStr[50];
            strftime(endTimeStr, sizeof(endTimeStr), "%Y-%m-%d-%H:%M:%S", localtime(&endTime));
            TimeNode* endTimeNode = queryTimestampHashTable(endTimeStr);
            if (!endTimeNode) {
                printf("End timestamp not found in the data structure.\n");
                return;
            }
        }
    }
    ARTNode* srcIPNode = NULL;
    ARTNode* dstIPNode = NULL;
    if (strcmp(src_IP, "ALL") != 0) {
        srcIPNode = queryART(artRoot, (uint8_t *)src_IP, strlen(src_IP), 0);
    }
    if (strcmp(dst_IP, "ALL") != 0) {
        dstIPNode = queryART(artRoot, (uint8_t *)dst_IP, strlen(dst_IP), 0);
    }
    ...
    HashTableEntry* protocolEntry = (protocolNum == -1) ? NULL : queryProtocolsHashTable(protocolNum);

    FlowNode* current = headFlow;
    while (current != NULL) {
        bool srcIPMatch = (strcmp(src_IP, "ALL") == 0) || ipMatchesPattern(current->src_IP, src_IP);
        bool dstIPMatch = (strcmp(dst_IP, "ALL") == 0) || ipMatchesPattern(current->dst_IP, dst_IP);
        ...
        if (srcIPMatch && dstIPMatch && srcPortMatch && dstPortMatch && protocolMatch) {
            ...
            while (data != NULL) {
                ...
                if ((useStartTime || dataTime >= startTime) && (useEndTime || dataTime <= endTime)) {
...
char** getUniqueFlows(int* flowCount) {
    int capacity = 10;
    char** flows = (char**)malloc(capacity * sizeof(char*));  // 确保这里声明了 flows
    if (flows == NULL) {
...
void queryAndSortFlowsWithTimeRange(time_t startTime, time_t endTime, int topN) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &end);
    double elapsed_seconds = end.tv_sec - start.tv_sec + (end.tv_nsec - start.tv_nsec) / 1e9;
    printf("Timequery took a total of %f seconds\n", elapsed_seconds);
    free(summaries);
}
...
void updateThresholds(const char* thresholdName, int newValue) {
    if (strcmp(thresholdName, "DATA_SIZE") == 0) {
        DATA_SIZE_THRESHOLD = newValue;
    } else if (strcmp(thresholdName, "FLOW_COUNT") == 0) {
        FLOW_COUNT_THRESHOLD = newValue;
    } else if (strcmp(thresholdName, "SRC_IP") == 0) {
        SRC_IP_THRESHOLD = newValue;
    } else if (strcmp(thresholdName, "DST_IP") == 0) {
        DST_IP_THRESHOLD = newValue;
    } else if (strcmp(thresholdName, "SRC_PORT") == 0) {
        SRC_PORT_THRESHOLD = newValue;
    } else if (strcmp(thresholdName, "DST_PORT") == 0) {
        DST_PORT_THRESHOLD = newValue;
    } else if (strcmp(thresholdName, "PROTOCOL") == 0) {
        PROTOCOL_THRESHOLD = newValue;
    } else {
        printf("Unknown threshold name: %s\n", thresholdName);
    }
}
...
while (running) {
    printf("Enter command (query, print, rank, timequery, set_time, set_threshold, exit, ): ");
	...
	//-----------------------------------------------------------------------------
	    ...
	    int newValue;
	    printf("Enter threshold name and new value (e.g., DATA_SIZE, FLOW_COUNT, SRC_IP, DST_IP): ");
	    scanf("%19s %d", thresholdName, &newValue);
	    updateThresholds(thresholdName, newValue);
	    printf("Threshold %s set to %d.\n", thresholdName, newValue);
	}
...
void printIPAddress(const char* nic) {
    ...
    for (iface = interfaces; iface != NULL; iface = iface->ifa_next) {
        ...
        if (iface->ifa_addr->sa_family == AF_INET) { // check it is IP4
            ...
            if (strcmp(iface->ifa_name, nic) == 0) {
                getnameinfo(iface->ifa_addr, sizeof(struct sockaddr_in), host, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
                printf("{ \"nic\": \"%s\", \"hostIP\": \"%s\" }\n", nic, host);
            }
        }
    }
...
void getHostIP(char* nic, char* hostIP) {
    struct ifaddrs *interfaces, *iface;
    if (getifaddrs(&interfaces) == -1) {
        perror("getifaddrs");
        exit(EXIT_FAILURE);
    ...
    for (iface = interfaces; iface != NULL; iface = iface->ifa_next) {
        if (iface->ifa_addr == NULL) continue;
        if (iface->ifa_addr->sa_family == AF_INET) { // check it is IP4
            // Ignore the loopback interface and choose the first active one
            if (!(iface->ifa_flags & IFF_LOOPBACK)) {
                strcpy(nic, iface->ifa_name);
                getnameinfo(iface->ifa_addr, sizeof(struct sockaddr_in), hostIP, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
                break; // Found a suitable NIC; exit the loop
            }
        }
    }
...
void queryAndSortFlowsToJson(int topN, char *jsonOutput, size_t jsonSize) {
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    currentFlow = globalFlowsHead;
    while (currentFlow != NULL) {
        int totalData = currentFlow->totalData;
    ...
    snprintf(jsonOutput, jsonSize, "{\"flows\":[");
    char entry[1024];
    for (int i = 0; i < summaryCount && i < topN; i++) {
        char src_country[100] = "Unknown", dst_country[100] = "Unknown";
        int gai_error, mmdb_error;

        MMDB_lookup_result_s src_result = MMDB_lookup_string(&mmdb, summaries[i].src_IP, &gai_error, &mmdb_error);
        if (mmdb_error == MMDB_SUCCESS) {
            MMDB_entry_data_s entry_data;
            if (MMDB_get_value(&src_result.entry, &entry_data, "country", "names", "en", NULL) == MMDB_SUCCESS && entry_data.has_data) {
                snprintf(src_country, sizeof(src_country), "%.*s", entry_data.data_size, entry_data.utf8_string);
        ...
        MMDB_lookup_result_s dst_result = MMDB_lookup_string(&mmdb, summaries[i].dst_IP, &gai_error, &mmdb_error);
        if (mmdb_error == MMDB_SUCCESS) {
            MMDB_entry_data_s entry_data;
            if (MMDB_get_value(&dst_result.entry, &entry_data, "country", "names", "en", NULL) == MMDB_SUCCESS && entry_data.has_data) {
                snprintf(dst_country, sizeof(dst_country), "%.*s", entry_data.data_size, entry_data.utf8_string);
        ...
        snprintf(entry, sizeof(entry), 
                 "{\"rank\":%d,\"srcIP\":\"%s\",\"srcPort\":%d,\"srcCountry\":\"%s\",\"dstIP\":\"%s\",\"dstPort\":%d,\"dstCountry\":\"%s\",\"protocol\":\"%s\",\"totalData\":%d}%s",
                 i + 1, summaries[i].src_IP, summaries[i].src_port, src_country,
                 summaries[i].dst_IP, summaries[i].dst_port, dst_country,
                 summaries[i].protocol, summaries[i].totalData,
                 i < (summaryCount - 1) && i < (topN - 1) ? "," : "");
        strncat(jsonOutput, entry, jsonSize - strlen(jsonOutput) - 1);
    }
    strncat(jsonOutput, "]}", jsonSize - strlen(jsonOutput) - 1);
...
void queryAndSortFlowsWithTimeRangeToJson(time_t startTime, time_t endTime, int topN, char *jsonOutput, size_t jsonSize) {
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    char *currentPtr = jsonOutput;
    size_t remainingSize = jsonSize;
    int length = snprintf(currentPtr, remainingSize, "{\"flows\":[");
    currentPtr += length;
    ...
    for (int i = 0; i < summaryCount && i < topN; i++) {
        ...
        length = snprintf(currentPtr, remainingSize,
                          "{\"rank\":%d,\"srcIP\":\"%s\",\"srcPort\":%d,\"srcCountry\":\"%s\","
                          "\"dstIP\":\"%s\",\"dstPort\":%d,\"dstCountry\":\"%s\","
                          "\"protocol\":\"%s\",\"totalData\":%d}%s",
                          i + 1, summaries[i].src_IP, summaries[i].src_port, src_country,
                          summaries[i].dst_IP, summaries[i].dst_port, dst_country,
                          summaries[i].protocol, summaries[i].totalData,
                          (i < topN - 1) ? "," : "");
        if (length < 0 || length >= remainingSize) {
            // Handle error or truncation
            fprintf(stderr, "Error or buffer too small while writing JSON output.\n");
            free(summaries);
            return; }
        currentPtr += length;
        remainingSize -= length;
...
void rankFlowsByCountInRangeToJson(time_t startTime, time_t endTime, int topN, char *jsonOutput, size_t jsonSize) {
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    for (int i = 0; i < topN && i < flowCount; i++) {
        ...
        snprintf(entry, sizeof(entry), 
                 "{\"rank\": %d, \"srcIP\": \"%s\", \"srcPort\": %d, \"srcCountry\": \"%s\", \"dstIP\": \"%s\", \"dstPort\": %d, \"dstCountry\": \"%s\", \"protocol\": \"%s\", \"flowCount\": %d}%s",
                 i + 1, diffs[i].flow->src_IP, diffs[i].flow->src_port, src_country,
                 diffs[i].flow->dst_IP, diffs[i].flow->dst_port, dst_country,
                 diffs[i].flow->protocol, diffs[i].flowCountDifference,
                 (i < topN - 1 && i < flowCount - 1) ? "," : ""); // Add comma except for last item
        strncat(jsonOutput, entry, jsonSize - strlen(jsonOutput) - 1);
    }
    strncat(jsonOutput, "]}", jsonSize - strlen(jsonOutput) - 1);
    clock_gettime(CLOCK_MONOTONIC, &end);
...
void rankAttributeWithinTimeRangeToJson(StatList *list, const char* attribute, time_t startTime, time_t endTime, int topN, char *jsonOutput, size_t jsonSize) {
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    snprintf(jsonOutput, jsonSize, "{\"attribute\":\"%s\",\"rankings\":[", attribute);
    char entry[1024]; // Buffer to hold individual JSON entry
    ...
    for (int i = 0; i < topN && i < tempList.size; i++) {
        ...
        snprintf(entry, sizeof(entry), 
                 "{\"rank\": %d, \"value\": \"%s\", \"country\": \"%s\", \"count\": %d}%s",
                 i + 1, tempList.items[i].value, country, tempList.items[i].count,
                 (i < topN - 1 && i < tempList.size - 1) ? "," : ""); // Add comma except for last item
        strncat(jsonOutput, entry, jsonSize - strlen(jsonOutput) - 1);
    }
     strncat(jsonOutput, "]}", jsonSize - strlen(jsonOutput) - 1);
    free(tempList.items);
...
void replace_space_with_dash(char* datetime) {
...
void rankAttributeToJson(StatList *list, const char *attributeType, int topN, char *jsonOutput, size_t jsonSize) {
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    snprintf(jsonOutput, jsonSize, "{\"attribute\":\"%s\",\"rankings\":[", attributeType);
    char entry[1024];
    ...
    for (int i = 0; i < topN && i < list->size; i++) {
        ...
        // Only perform the country lookup for IP attributes
        if (strcmp(attributeType, "src_ip") == 0 || strcmp(attributeType, "dst_ip") == 0) {
            MMDB_lookup_result_s result = MMDB_lookup_string(&mmdb, list->items[i].value, &gai_error, &mmdb_error);
            if (mmdb_error == MMDB_SUCCESS) {
                MMDB_entry_data_s entry_data;
            ...
            snprintf(entry, sizeof(entry), "{\"rank\":%d,\"value\":\"%s\",\"country\":\"%s\",\"count\":%d}%s",
                     i + 1, list->items[i].value, country, list->items[i].count, i < (list->size - 1) && i < (topN - 1) ? "," : "");
            printf("Top #%d: %s (%s) - Count: %d\n", i + 1, list->items[i].value, country, list->items[i].count); // Console output
        } else {
            snprintf(entry, sizeof(entry), "{\"rank\":%d,\"value\":\"%s\",\"count\":%d}%s",
                     i + 1, list->items[i].value, list->items[i].count, i < (list->size - 1) && i < (topN - 1) ? "," : "");
            printf("%s: %d\n", list->items[i].value, list->items[i].count); // Console output
        }
        strncat(jsonOutput, entry, jsonSize - strlen(jsonOutput) - 1);
    }
    strncat(jsonOutput, "]}", jsonSize - strlen(jsonOutput) - 1);
    ...
    printf("The ranking operation took %.9f seconds.\n", time_taken); // Console output
}

void rankFlowsByFlowCountToJson(int topN, char *jsonOutput, size_t jsonSize) {
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    // Allocate an array for the flow nodes
    FlowNode** flowArray = (FlowNode**)malloc(count * sizeof(FlowNode*));
    if (!flowArray) {
        perror("Memory allocation failed");
        return;
    ...
    // Sort the flow array based on flow count
    qsort(flowArray, count, sizeof(FlowNode*), compareFlowCount);
    ...
    for (int i = 0; i < topN && i < count; i++) {
        ...
        // Lookup source IP country
        MMDB_lookup_result_s src_result = MMDB_lookup_string(&mmdb, flowArray[i]->src_IP, &gai_error, &mmdb_error);
        if (mmdb_error == MMDB_SUCCESS) {
            MMDB_entry_data_s entry_data;
            if (MMDB_get_value(&src_result.entry, &entry_data, "country", "names", "en", NULL) == MMDB_SUCCESS && entry_data.has_data) {
                snprintf(src_country, sizeof(src_country), "%.*s", entry_data.data_size, entry_data.utf8_string);
        ...
        // Lookup destination IP country
        MMDB_lookup_result_s dst_result = MMDB_lookup_string(&mmdb, flowArray[i]->dst_IP, &gai_error, &mmdb_error);
        if (mmdb_error == MMDB_SUCCESS) {
            MMDB_entry_data_s entry_data;
            if (MMDB_get_value(&dst_result.entry, &entry_data, "country", "names", "en", NULL) == MMDB_SUCCESS && entry_data.has_data) {
                snprintf(dst_country, sizeof(dst_country), "%.*s", entry_data.data_size, entry_data.utf8_string);
        ...
        // Build JSON entry
        snprintf(entry, sizeof(entry), "{\"rank\":%d,\"srcIP\":\"%s\",\"srcPort\":%d,\"srcCountry\":\"%s\",\"dstIP\":\"%s\",\"dstPort\":%d,\"dstCountry\":\"%s\",\"protocol\":\"%s\",\"flowCount\":%d}%s",
                 i + 1, flowArray[i]->src_IP, flowArray[i]->src_port, src_country,
                 flowArray[i]->dst_IP, flowArray[i]->dst_port, dst_country,
                 flowArray[i]->protocol, flowArray[i]->flowCount, i < (count - 1) && i < (topN - 1) ? "," : "");
        strncat(jsonOutput, entry, jsonSize - strlen(jsonOutput) - 1);
    }
    strncat(jsonOutput, "]}", jsonSize - strlen(jsonOutput) - 1);
    ...
    free(flowArray);  // Release the array memory
}

enum MHD_Result handle_time_range_rank_request(void *cls, struct MHD_Connection *connection) {
    const char *attribute = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "attribute");
    const char *topN_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "topN");
    const char *startTime_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "startTime");
    const char *endTime_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "endTime");
    

    // Validate the input parameters
     if (attribute == NULL || topN_str == NULL || startTime_str == NULL || endTime_str == NULL) {
        return MHD_NO; // 必须提供所有参数
    }
    
     int topN = atoi(topN_str);
    char formattedStartTimeStr[20];
    char formattedEndTimeStr[20];
    strncpy(formattedStartTimeStr, startTime_str, 19);
    strncpy(formattedEndTimeStr, endTime_str, 19);
    formattedStartTimeStr[19] = '\0';
    formattedEndTimeStr[19] = '\0';
    
    for (int i = 0; formattedStartTimeStr[i] != '\0'; i++) {
        if (formattedStartTimeStr[i] == ' ') {
            formattedStartTimeStr[i] = '-';
        }
    }

    for (int i = 0; formattedEndTimeStr[i] != '\0'; i++) {
        if (formattedEndTimeStr[i] == ' ') {
            formattedEndTimeStr[i] = '-';
        }
    }

    // Parse the formatted time strings into struct tm
    struct tm startTimeTm;
    struct tm endTimeTm;
    memset(&startTimeTm, 0, sizeof(struct tm));
    memset(&endTimeTm, 0, sizeof(struct tm));

    if (strptime(formattedStartTimeStr, "%Y-%m-%d-%H:%M:%S", &startTimeTm) == NULL ||
        strptime(formattedEndTimeStr, "%Y-%m-%d-%H:%M:%S", &endTimeTm) == NULL) {
        return MHD_NO; // 时间字符串格式错误
    }

    // Convert the struct tm to time_t
    time_t startTime = mktime(&startTimeTm);
    time_t endTime = mktime(&endTimeTm);

    if (startTime == (time_t)(-1) || endTime == (time_t)(-1)) {
        return MHD_NO; // 时间转换错误
    }



    char response_text[8192]; // Buffer to hold JSON response
    memset(response_text, 0, sizeof(response_text));

    if (strcmp(attribute, "flow") == 0) {
		queryAndSortFlowsWithTimeRangeToJson(startTime, endTime, topN, response_text, sizeof(response_text));
	    } else  if (strcmp(attribute, "flowcount") == 0) {
		rankFlowsByCountInRangeToJson(startTime, endTime, topN, response_text, sizeof(response_text));
	    } else  if (strcmp(attribute, "src_ip") == 0) {
		rankAttributeWithinTimeRangeToJson(&srcIPList, attribute, startTime, endTime, topN, response_text, sizeof(response_text));
	    } else if (strcmp(attribute, "dst_ip") == 0) {
		rankAttributeWithinTimeRangeToJson(&dstIPList, attribute, startTime, endTime, topN, response_text, sizeof(response_text));
	    } else if (strcmp(attribute, "src_port") == 0) {
		rankAttributeWithinTimeRangeToJson(&srcPortList, attribute, startTime, endTime, topN, response_text, sizeof(response_text));
	    } else if (strcmp(attribute, "dst_port") == 0) {
		rankAttributeWithinTimeRangeToJson(&dstPortList, attribute, startTime, endTime, topN, response_text, sizeof(response_text));
	    } else if (strcmp(attribute, "protocol") == 0) {
		rankAttributeWithinTimeRangeToJson(&protocolList, attribute, startTime, endTime, topN, response_text, sizeof(response_text));
	    } else {
        return MHD_NO; // Invalid attribute
    }

    // 发送响应
    struct MHD_Response *response = MHD_create_response_from_buffer(strlen(response_text), (void*)response_text, MHD_RESPMEM_MUST_COPY);
...
enum MHD_Result answer_rank_request(void *cls, struct MHD_Connection *connection) {
    const char *attribute = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "attribute");
    const char *topN_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "topN");
    if (attribute == NULL || topN_str == NULL) {
        return MHD_NO; // Must provide both 'attribute' and 'topN' parameters
    }
    ...
    if (strcmp(attribute, "flow") == 0) {
        queryAndSortFlowsToJson(topN, response_text, sizeof(response_text));
    } else if (strcmp(attribute, "src_ip") == 0) {
        rankAttributeToJson(&srcIPList,"src_ip", topN, response_text, sizeof(response_text));
    } else if (strcmp(attribute, "dst_ip") == 0) {
        rankAttributeToJson(&dstIPList,"dst_ip", topN, response_text, sizeof(response_text));
    } else if (strcmp(attribute, "src_port") == 0) {
       rankAttributeToJson(&srcPortList,"src_port", topN, response_text, sizeof(response_text));
    } else if (strcmp(attribute, "dst_port") == 0) {
       rankAttributeToJson(&dstPortList,"dst_port", topN, response_text, sizeof(response_text));
    } else if (strcmp(attribute, "protocol") == 0) {
        rankAttributeToJson(&protocolList,"protocol", topN, response_text, sizeof(response_text));
    } else if (strcmp(attribute, "flowcount") == 0) {
        rankFlowsByFlowCountToJson(topN, response_text, sizeof(response_text));
    } else {
        return MHD_NO; // Invalid attribute
    }
...
enum MHD_Result answer_traffic_size(void *cls, struct MHD_Connection *connection) {
    extern TimeNode *lastTime;


    char response_text[1024];
    int snprintf_result = snprintf(response_text, sizeof(response_text),
                                   "{\"trafficSize\": %" PRIu64 ", \"intervalSeconds\": %d, \"totalTrafficSize\": %" PRIu64 "}",
                                   lastTime->trafficSizeDifference, intervalSeconds, totalTrafficSize);
...
static int parse_log_line(cJSON *json_array, char *line) {
    // Parse the log line, safely tokenize and create JSON objects
    char *timestamp = strtok(line, " ");
    if (!timestamp) return -1; // Error handling
    char *systemName = strtok(NULL, " ");
    ...
    cJSON_AddStringToObject(log_entry, "systemName", systemName);
    cJSON_AddStringToObject(log_entry, "message", message);
    cJSON_AddItemToArray(json_array, log_entry);
    return 0;
}

static enum MHD_Result send_response(struct MHD_Connection *connection, unsigned int status_code, struct MHD_Response *response) {
    MHD_add_response_header(response, "Content-Type", "application/json");
    MHD_add_response_header(response, "Access-Control-Allow-Origin", "*");
    MHD_add_response_header(response, "Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    MHD_add_response_header(response, "Access-Control-Allow-Headers", "Content-Type, X-Requested-With");
...
enum MHD_Result answer_logs_request(void *cls, struct MHD_Connection *connection) {
    const char *filename = "Flow_Alarm_Log.txt";
    FILE *file = fopen(filename, "r");
    if (!file) {
        const char *error_msg = "{\"error\":\"Unable to open log file.\"}";
        struct MHD_Response *response = MHD_create_response_from_buffer(strlen(error_msg),
                                                                        (void *)error_msg, 
                                                                        MHD_RESPMEM_MUST_COPY);
        MHD_add_response_header(response, "Content-Type", "application/json");
        MHD_add_response_header(response, "Access-Control-Allow-Origin", "*");
        return send_response(connection, MHD_HTTP_INTERNAL_SERVER_ERROR, response);
    }

    char line[1024];
    cJSON *json_array = cJSON_CreateArray();
    while (fgets(line, sizeof(line), file) != NULL) {
        if (parse_log_line(json_array, line) != 0) {
            continue;  // Optionally handle malformed lines or abort altogether
        }
    }
    fclose(file);
...
cJSON *convert_flows_to_json(FlowNode *flowNode) {
    cJSON *flows = cJSON_CreateArray();
    while (flowNode != NULL) {
    cJSON *flow_json = cJSON_CreateObject();
    // Ensure all string operations are safe
    if (flowNode->src_IP != NULL) cJSON_AddStringToObject(flow_json, "src_IP", flowNode->src_IP);
    if (flowNode->dst_IP != NULL) cJSON_AddStringToObject(flow_json, "dst_IP", flowNode->dst_IP);
    cJSON_AddNumberToObject(flow_json, "src_port", flowNode->src_port);
    cJSON_AddNumberToObject(flow_json, "dst_port", flowNode->dst_port);
    if (flowNode->protocol != NULL) cJSON_AddStringToObject(flow_json, "protocol", flowNode->protocol);
    cJSON_AddNumberToObject(flow_json, "flowCount", flowNode->flowCount);

    cJSON *datas = cJSON_CreateArray();
    DataNode *dataNode = flowNode->datas;
    while (dataNode != NULL) {
        cJSON *data_json = cJSON_CreateObject();
        cJSON_AddNumberToObject(data_json, "single_data", dataNode->single_data);
        cJSON_AddNumberToObject(data_json, "data", dataNode->data);
        cJSON_AddNumberToObject(data_json, "single_flowCount", dataNode->single_flowCount);
        cJSON_AddNumberToObject(data_json, "flowCount", dataNode->flowCount);
        cJSON_AddItemToArray(datas, data_json);
        dataNode = dataNode->nextData;
    }
    cJSON_AddItemToObject(flow_json, "datas", datas);
    cJSON_AddItemToArray(flows, flow_json);
    flowNode = flowNode->nextFlow;
}
    return flows;
}
...
// 函数转换时间节点到JSON
cJSON *convert_to_json(TimeNode *headTime) {
if (headTime == NULL) {
    fprintf(stderr, "Received NULL pointer for headTime.\n");
    return NULL; // Exit early or handle the error as appropriate.
}

    cJSON *root = cJSON_CreateObject();
    cJSON *times = cJSON_AddArrayToObject(root, "times");

    TimeNode *current_time = headTime;
    if (current_time != NULL) {
    do {
        cJSON *time_json = cJSON_CreateObject();
        cJSON_AddItemToObject(time_json, "timestamp", cJSON_CreateString(current_time->timestamp));
        cJSON_AddItemToObject(time_json, "flows", convert_flows_to_json(current_time->flows));
        cJSON_AddItemToArray(times, time_json);
        current_time = current_time->nextTime;
    } while (current_time != headTime);
}
    return root;
}


enum MHD_Result answer_set_interval_request(void *cls, struct MHD_Connection *connection,
                                            const char *url, const char *method,
                                            const char *version, const char *upload_data,
                                            size_t *upload_data_size, void **con_cls) {
    static int new_interval = 0;
    ...
    if (*upload_data_size != 0) {
        new_interval = atoi(upload_data);
        if (new_interval > 0) {
            intervalSeconds = new_interval;
            *upload_data_size = 0; // Indicate that all data has been processed
            const char *post_response_text = "Interval updated successfully";
            struct MHD_Response *post_response = MHD_create_response_from_buffer(
                strlen(post_response_text),
                (void *)post_response_text,
                MHD_RESPMEM_MUST_COPY);
            MHD_add_response_header(post_response, "Access-Control-Allow-Origin", "*");
            MHD_add_response_header(post_response, "Access-Control-Allow-Methods", "POST, GET, OPTIONS");
            MHD_add_response_header(post_response, "Access-Control-Allow-Headers", "Content-Type");
            enum MHD_Result ret = MHD_queue_response(connection, MHD_HTTP_OK, post_response);
            MHD_destroy_response(post_response);
            return ret;
        } else {
            const char *error_response_text = "Invalid interval value";
            struct MHD_Response *error_response = MHD_create_response_from_buffer(
                strlen(error_response_text),
                (void *)error_response_text,
                MHD_RESPMEM_MUST_COPY);
            MHD_add_response_header(error_response, "Access-Control-Allow-Origin", "*");
            MHD_add_response_header(error_response, "Access-Control-Allow-Methods", "POST, GET, OPTIONS");
            MHD_add_response_header(error_response, "Access-Control-Allow-Headers", "Content-Type");
            enum MHD_Result ret = MHD_queue_response(connection, MHD_HTTP_BAD_REQUEST, error_response);
...
const char* get_origin(struct MHD_Connection *connection) {
    const char *origin = MHD_lookup_connection_value(connection, MHD_HEADER_KIND, "Origin");
    return origin ? origin : "";
}

void updateThresholds(const char *key, const char *value) {
    int intValue = atoi(value);
    if (intValue <= 0) {
        return;
    }

    if (strcmp(key, "DATA_SIZE") == 0) {
        DATA_SIZE_THRESHOLD = intValue;
    } else if (strcmp(key, "FLOW_COUNT") == 0) {
        FLOW_COUNT_THRESHOLD = intValue;
    } else if (strcmp(key, "SRC_IP") == 0) {
        SRC_IP_THRESHOLD = intValue;
    } else if (strcmp(key, "DST_IP") == 0) {
        DST_IP_THRESHOLD = intValue;
    } else if (strcmp(key, "SRC_PORT") == 0) {
        SRC_PORT_THRESHOLD = intValue;
    } else if (strcmp(key, "DST_PORT") == 0) {
        DST_PORT_THRESHOLD = intValue;
    } else if (strcmp(key, "PROTOCOL") == 0) {
        PROTOCOL_THRESHOLD = intValue;
    }
}

enum MHD_Result answer_thresholds_request(void *cls, struct MHD_Connection *connection,
                                          const char *url, const char *method,
                                          const char *version, const char *upload_data,
                                          size_t *upload_data_size, void **con_cls) {
    static char *new_thresholds = NULL;
    printf("%s\n", method);
    ...
    if (strcmp(method, "GET") == 0) {
        // Construct a JSON string with the current threshold values
        char response_text[1024];
        snprintf(response_text, sizeof(response_text),
                 "{\"DATA_SIZE\": %d, \"FLOW_COUNT\": %d, \"SRC_IP\": %d, \"DST_IP\": %d, \"SRC_PORT\": %d, \"DST_PORT\": %d, \"PROTOCOL\": %d}",
                 DATA_SIZE_THRESHOLD, FLOW_COUNT_THRESHOLD, SRC_IP_THRESHOLD, DST_IP_THRESHOLD, SRC_PORT_THRESHOLD, DST_PORT_THRESHOLD, PROTOCOL_THRESHOLD);
    ...
    if (*upload_data_size != 0) {
        ...
        if (json == NULL) {
            const char *error_response_text = "Invalid JSON format";
            struct MHD_Response *error_response = MHD_create_response_from_buffer(
                strlen(error_response_text), (void *)error_response_text, MHD_RESPMEM_MUST_COPY);
            MHD_add_response_header(error_response, "Access-Control-Allow-Origin", origin);
            MHD_add_response_header(error_response, "Access-Control-Allow-Methods", "POST, GET, OPTIONS");
            MHD_add_response_header(error_response, "Access-Control-Allow-Headers", "Content-Type");
            MHD_add_response_header(error_response, "Access-Control-Allow-Credentials", "true");
            enum MHD_Result ret = MHD_queue_response(connection, MHD_HTTP_BAD_REQUEST, error_response);
            MHD_destroy_response(error_response);
            return ret;
        }
        ...
        const char *post_response_text = "Thresholds updated successfully";
        struct MHD_Response *post_response = MHD_create_response_from_buffer(
            strlen(post_response_text), (void *)post_response_text, MHD_RESPMEM_MUST_COPY);
        MHD_add_response_header(post_response, "Access-Control-Allow-Origin", origin);
        MHD_add_response_header(post_response, "Access-Control-Allow-Methods", "POST, GET, OPTIONS");
        MHD_add_response_header(post_response, "Access-Control-Allow-Headers", "Content-Type");
        MHD_add_response_header(post_response, "Access-Control-Allow-Credentials", "true");
        enum MHD_Result ret = MHD_queue_response(connection, MHD_HTTP_OK, post_response);
        MHD_destroy_response(post_response);
        return ret;
    }
...
void clearAlerts() {
    pthread_mutex_lock(&alertsMutex);
    Alert* current = alertsHead;
    while (current != NULL) {
        Alert* next = current->next;
        free(current);
        current = next;
    }
    alertsHead = NULL;
...
enum MHD_Result answer_alerts_request(void *cls, struct MHD_Connection *connection) {
    pthread_mutex_lock(&alertsMutex);
    cJSON *json = cJSON_CreateArray();
    Alert* current = alertsHead;
    while (current != NULL) {
        cJSON *alert = cJSON_CreateObject();
        cJSON_AddStringToObject(alert, "timestamp", current->timestamp);
        cJSON_AddStringToObject(alert, "alertType", current->alertType);
        cJSON_AddStringToObject(alert, "message", current->message);
        cJSON_AddItemToArray(json, alert);
    ...
    clearAlerts(); // Clear the alerts list after sending the response

    return ret;
}

void queryResultsToJson(FlowNode* headFlow, const char* src_IP, const char* src_port_str, const char* dst_IP, const char* dst_port_str, const char* protocol, time_t startTime, time_t endTime, bool useStartTime, bool useEndTime, char* jsonOutput, size_t jsonSize) {
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    while (current != NULL) {
        ...
        if (srcIPMatch && dstIPMatch && srcPortMatch && dstPortMatch && protocolMatch) {
            ...
            // Create JSON object for this flow
            json_object *jflow = json_object_new_object();
            json_object_object_add(jflow, "srcIP", json_object_new_string(current->src_IP));
            json_object_object_add(jflow, "srcPort", json_object_new_int(current->src_port));
            json_object_object_add(jflow, "srcCountry", json_object_new_string(src_country));
            json_object_object_add(jflow, "dstIP", json_object_new_string(current->dst_IP));
            json_object_object_add(jflow, "dstPort", json_object_new_int(current->dst_port));
            json_object_object_add(jflow, "dstCountry", json_object_new_string(dst_country));
            json_object_object_add(jflow, "protocol", json_object_new_string(current->protocol));

            // Create JSON array for the data points
            json_object *jdata_array = json_object_new_array();
            DataNode* data = current->datas;
            while (data != NULL) {
                time_t dataTime = convertStringToTime(data->time->timestamp);
               // printf("Data timestamp: %s, Data time_t: %ld, Start time: %ld, End time: %ld\n", data->time->timestamp, dataTime, startTime, endTime);
                if ((dataTime >= startTime) && (dataTime <= endTime)) {
                    json_object *jdata = json_object_new_object();
                    json_object_object_add(jdata, "timestamp", json_object_new_string(data->time->timestamp));
                    json_object_object_add(jdata, "singleDataSize", json_object_new_int(data->single_data));
                    json_object_array_add(jdata_array, jdata);
                }
                data = data->nextData;
            }

            // Add data array to the flow JSON object
            json_object_object_add(jflow, "data", jdata_array);
            json_object_array_add(jarray, jflow);
        }
        current = current->nextFlow;
    }

    json_object_object_add(jobj, "flows", jarray);
    const char* jsonString = json_object_to_json_string(jobj);
    snprintf(jsonOutput, jsonSize, "%s", jsonString);
...
enum MHD_Result handle_query_request(void* cls, struct MHD_Connection* connection,
                                     const char* url, const char* method, const char* version,
                                     const char* upload_data, size_t* upload_data_size, void** con_cls) {
    if (strcmp(method, "GET") != 0) {
        return MHD_NO; // Only support GET requests
    }

    const char* src_IP = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "src_ip");
    const char* src_port_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "src_port");
    const char* dst_IP = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "dst_ip");
    const char* dst_port_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "dst_port");
    const char* protocol = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "protocol");
    const char* startTime_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "start_time");
    const char* endTime_str = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "end_time");

    // Validate input parameters
    if (!src_IP || !src_port_str || !dst_IP || !dst_port_str || !protocol) {
        return MHD_NO; // Missing parameters
    }
    ...
    time_t startTime = firstPacketTime;
    time_t endTime = lastPacketTime;
    bool useStartTime = false;
    bool useEndTime = false;

    if (startTime_str && strlen(startTime_str) > 0) {
        struct tm tm;
        memset(&tm, 0, sizeof(struct tm));
        if (strptime(startTime_str, "%Y-%m-%d %H:%M:%S", &tm) != NULL) {
            startTime = mktime(&tm);
            useStartTime = true;
        } else {
            printf("Failed to parse start time: %s\n", startTime_str);
        }
    }

    if (endTime_str && strlen(endTime_str) > 0) {
        struct tm tm;
        memset(&tm, 0, sizeof(struct tm));
        if (strptime(endTime_str, "%Y-%m-%d %H:%M:%S", &tm) != NULL) {
            endTime = mktime(&tm);
            useEndTime = true;
        } else {
            printf("Failed to parse end time: %s\n", endTime_str);
        }
    }

    // Query the flows
    char jsonOutput[8192];
    queryResultsToJson(globalFlowsHead, src_IP, src_port_str, dst_IP, dst_port_str, protocol, startTime, endTime, useStartTime, useEndTime, jsonOutput, sizeof(jsonOutput));

    // Create and send the response
    struct MHD_Response* response = MHD_create_response_from_buffer(strlen(jsonOutput), jsonOutput, MHD_RESPMEM_MUST_COPY);
    MHD_add_response_header(response, "Content-Type", "application/json");
    MHD_add_response_header(response, "Access-Control-Allow-Origin", "*");
    enum MHD_Result ret = MHD_queue_response(connection, MHD_HTTP_OK, response);
    MHD_destroy_response(response);
...
enum MHD_Result answer_to_connection(void *cls, struct MHD_Connection *connection,
    ...
    if (strcmp(url, "/api/setInterval") == 0) {
        return answer_set_interval_request(cls, connection, url, method, version, upload_data, upload_data_size, con_cls);
    }
    
    if (strcmp(url, "/api/query") == 0) {
        return handle_query_request(cls, connection, url, method, version, upload_data, upload_data_size, con_cls);
    }
    
     if (strcmp(url, "/api/thresholds") == 0) {
        return answer_thresholds_request(cls, connection, url, method, version, upload_data, upload_data_size, con_cls);
    }
    
    if (strcmp(url, "/api/logs") == 0) {
    return answer_logs_request(cls, connection);
    }
    
    if (strcmp(url, "/api/alerts") == 0) {
        return answer_alerts_request(cls, connection);
    }
    
    if (strcmp(method, "GET") != 0)
        return MHD_NO; // 只处理GET请求
   
    // Handle requests for the traffic data
    if (strcmp(url, "/api/traffic") == 0) {
        return answer_traffic_size(cls, connection);
    }
    
    if (strcmp(url, "/api/rank") == 0) {
    return answer_rank_request(cls, connection);
}
    if (strcmp(url, "/api/time-range-ranking") == 0) {
    return handle_time_range_rank_request(cls, connection);
}

    // Handle requests for the totalTrafficSize data
    else if (strcmp(url, "/api/totalTrafficSize") == 0) {
        // Create JSON response with totalTrafficSize
        snprintf(response_text, sizeof(response_text), "{\"totalTrafficSize\": %" PRIu64 "}", totalTrafficSize);
    ...
    // Handle other requests and show NIC and Host IP
    else {
        char nic[NI_MAXHOST] = "";
        char hostIP[NI_MAXHOST] = "";
        getHostIP(nic, hostIP);
        if (snprintf(response_text, sizeof(response_text), "%s\n%s\n", nic, hostIP) >= sizeof(response_text)) {
            fprintf(stderr, "Error: response_text buffer too small\n");
            exit(EXIT_FAILURE);
        }
...
void* run_http_daemon(void *arg) {
    ...
    // Keep the thread running to maintain the HTTP server
...
int main() {
    ...
    char dev[] = "enp5s0"; 
    //char dev[] = "enp0s3"; 
    pthread_t captureThread, displayThread, monitorThread, httpThread;;
    ...
    if (pthread_create(&httpThread, NULL, run_http_daemon, NULL) != 0) {
        fprintf(stderr, "Error creating HTTP daemon thread\n");
        return 2;
    }
