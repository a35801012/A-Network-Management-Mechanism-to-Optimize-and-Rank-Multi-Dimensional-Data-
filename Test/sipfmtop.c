//gcc -o sipfmtop sipfmtop.c siphash.c -lpcap -lmaxminddb -lpthread 
//sipfmtop.c
//9
...
uint32_t calculateHash(const void *data, size_t len) {
    uint8_t hashOutput[8]; // SipHash 輸出是 64 位元（8 bytes）
    siphash(data, len, siphashKey, hashOutput, sizeof(hashOutput));
    return *((uint32_t*)hashOutput); // 使用前 4 bytes 作為哈希值
}
...
void initializeTimestampHashTable() {
    timestampHashTable = calloc(TIMESTAMP_HASH_TABLE_SIZE, sizeof(TimestampTableEntry*));
...
void resizeTimestampHashTable() {
    int newSize = 2 * TIMESTAMP_HASH_TABLE_SIZE;  // Determine new size based on your requirements
    TimestampTableEntry** newTable = calloc(newSize, sizeof(TimestampTableEntry*));
...
void packetHandler(u_char *userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    */
    ...
    siphash(srcIP, strlen(srcIP), siphashKey, hashOutput, sizeof(hashOutput));
    uint32_t srcIPHash = *((uint32_t*)hashOutput); // Using the first 4 bytes

    // SipHash for dstIP
    siphash(dstIP, strlen(dstIP), siphashKey, hashOutput, sizeof(hashOutput));
    uint32_t dstIPHash = *((uint32_t*)hashOutput); // Using the first 4 bytes

    // SipHash for srcPort
    siphash(&srcPort, sizeof(srcPort), siphashKey, hashOutput, sizeof(hashOutput));
    uint32_t srcPortHash = *((uint32_t*)hashOutput); // Using the first 4 bytes

    // SipHash for dstPort
    siphash(&dstPort, sizeof(dstPort), siphashKey, hashOutput, sizeof(hashOutput));
    uint32_t dstPortHash = *((uint32_t*)hashOutput); // Using the first 4 bytes

    // SipHash for protocol
    siphash(&ipHeader->ip_p, sizeof(ipHeader->ip_p), siphashKey, hashOutput, sizeof(hashOutput));
    uint32_t protocolHash = *((uint32_t*)hashOutput); // Using the first 4 bytes
