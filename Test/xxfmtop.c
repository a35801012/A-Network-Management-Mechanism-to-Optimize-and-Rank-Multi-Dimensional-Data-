//gcc -o xxfmtop xxfmtop.c xxhash.c -lpcap -lmaxminddb -lpthread 
...
TimeNode* queryTimestampHashTable(const char* timestamp) {
    XXH64_hash_t hashValue = XXH64(timestamp, strlen(timestamp), HASH_SEED);
...
void packetHandler(u_char *userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    */
    ...
    XXH64_hash_t srcIPHash, dstIPHash;
    XXH64_hash_t protocolHashValue, srcPortHashValue, dstPortHashValue;
    ...
    srcIPHash = XXH64(srcIP, strlen(srcIP), HASH_SEED);
    dstIPHash = XXH64(dstIP, strlen(dstIP), HASH_SEED);
    srcPortHashValue = XXH64(&srcPort, sizeof(srcPort), HASH_SEED);
    dstPortHashValue = XXH64(&dstPort, sizeof(dstPort), HASH_SEED);
    protocolHashValue = XXH64(&ipHeader->ip_p, sizeof(ipHeader->ip_p), HASH_SEED);
...
FlowNode* findFlow(char* src_IP, char* dst_IP, int src_port, int dst_port, char* protocol) {
    ...
    uint32_t srcPortHash = calculateHash((void*)&src_port, sizeof(src_port));
    uint32_t dstPortHash = calculateHash((void*)&dst_port, sizeof(dst_port));
...
FlowNode* findOrAddFlow(char* src_IP, char* dst_IP, int src_port, int dst_port, char* protocol) {
    ...
    newNode->src_port_hash = calculateHash((void*)&src_port, sizeof(src_port));
    newNode->dst_port_hash = calculateHash((void*)&dst_port, sizeof(dst_port));
