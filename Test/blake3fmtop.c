//gcc -o blake3fmtop blake3fmtop.c blake3.h libblake3.a -lpcap -lmaxminddb -lpthread 
...
uint32_t calculateHash(const void *input, size_t length) {
    uint32_t hashValue;
...
TimeNode* queryTimestampHashTable(const char* timestamp) {
    uint32_t hashValue;
    hashWithBlake3(timestamp, strlen(timestamp), &hashValue);
...
void packetHandler(u_char *userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    */
    ...
    uint32_t srcIPHash = calculateHash(srcIP, strlen(srcIP));
    uint32_t dstIPHash = calculateHash(dstIP, strlen(dstIP));
    uint32_t srcPortHash = calculateHash(&srcPort, sizeof(srcPort));
    uint32_t dstPortHash = calculateHash(&dstPort, sizeof(dstPort));
    uint32_t protocolHash = calculateHash(protocolStr, strlen(protocolStr));
