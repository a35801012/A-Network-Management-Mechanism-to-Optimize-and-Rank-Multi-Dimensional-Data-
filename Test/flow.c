...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    printf("Time: %s.%06ld\n", timestr, (long)header->ts.tv_usec);
    printf("TTL: %d\n", ip_header->ttl);
   ...
   switch (ip_header->protocol) {
        case IPPROTO_TCP:
            print_tcp_packet(packet, ip_header);
            break;
        case IPPROTO_UDP:
            print_udp_packet(packet, ip_header);
            break;
        case IPPROTO_ICMP:
            print_icmp_packet(packet, ip_header);
            break;
        default:
            printf("The Other Protocol Number: %d\n", ip_header->protocol);
            break;
    }
    printf("-------------------------------------------\n");
}
