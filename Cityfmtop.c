//g++ Cityfmtop.c -o cityfmtop -lcityhash -lpcap -lmaxminddb -lpthread 
//Cityfmtop.c
//10
...
uint64_t calculateHash(const char *data, size_t len) {
    return CityHash64(data, len);
}

uint64_t calculateHashForInt(int value) {
    return calculateHash((const char*)&value, sizeof(value));
}
...
void packetHandler(u_char *userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    */
    ...
    uint64_t srcPortHash, dstPortHash, protocolHash;
...
FlowNode* findFlow(char* src_IP, char* dst_IP, int src_port, int dst_port, char* protocol) {
    uint32_t srcIPHash = calculateHash(src_IP, strlen(src_IP));
    uint32_t dstIPHash = calculateHash(dst_IP, strlen(dst_IP));
    uint32_t srcPortHash = calculateHashForInt(src_port);
    uint32_t dstPortHash = calculateHashForInt(dst_port);
...
FlowNode* findOrAddFlow(char* src_IP, char* dst_IP, int src_port, int dst_port, char* protocol) {
    ...
    newNode->src_IP_hash = calculateHash(src_IP, strlen(src_IP));
    newNode->dst_IP_hash = calculateHash(dst_IP, strlen(dst_IP));
    newNode->src_port_hash = calculateHashForInt(src_port);
    newNode->dst_port_hash = calculateHashForInt(dst_port);
...
void queryFlow(FlowNode* headFlow, const char* src_IP, const char* src_port_str, const char* dst_IP, const char* dst_port_str, const char* protocol, time_t startTime, time_t endTime, bool useStartTime, bool useEndTime) {
    ...
    // 使用 useStartTime 和 useEndTime 参数来决定是否需要查询 TimeNode
    ...
    while (current != NULL) {
        bool srcIPMatch = (srcIPNode == NULL || (srcIPNode->isLeaf && srcIPNode->hashValue == current->src_IP_hash));
        bool dstIPMatch = (dstIPNode == NULL || (dstIPNode->isLeaf && dstIPNode->hashValue == current->dst_IP_hash));
        ...
        bool protocolMatch = (protocolEntry == NULL || protocolEntry->hashValue == current->protocol_hash);
...
void queryFlow(FlowNode* headFlow, const char* src_IP, const char* src_port_str, const char* dst_IP, const char* dst_port_str, const char* protocol, time_t startTime, time_t endTime, bool useStartTime, bool useEndTime) {
    ...
    FlowNode* current = headFlow;
    while (current != NULL) {
        bool srcIPMatch = (strcmp(src_IP, "ALL") == 0 || strcmp(src_IP, current->src_IP) == 0);
        bool dstIPMatch = (strcmp(dst_IP, "ALL") == 0 || strcmp(dst_IP, current->dst_IP) == 0);
        bool srcPortMatch = (src_port == -1 || current->src_port == src_port);
        bool dstPortMatch = (dst_port == -1 || current->dst_port == dst_port);
        bool protocolMatch = (strcmp(protocol, "ALL") == 0 || strcmp(protocol, current->protocol) == 0);
