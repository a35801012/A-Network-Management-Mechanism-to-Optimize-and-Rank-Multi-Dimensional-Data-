//gcc -o murtop murmurfmtop.c  MurmurHash3.o -lpcap -lmaxminddb -lpthread 
...
TimeNode* queryTimestampHashTable(const char* timestamp) {
    uint32_t hashValue;
    MurmurHash3_x86_32(timestamp, strlen(timestamp), HASH_SEED, &hashValue);
...
void insertIntoTimestampHashTable(const char* timestamp, TimeNode* timeNode) {
    ...
    MurmurHash3_x64_128(timestamp, strlen(timestamp), HASH_SEED, hashValue);
    int bucket = hashValue[0] % TIMESTAMP_HASH_TABLE_SIZE;
    ...
    newEntry->hashValue = hashValue[0];
...
void packetHandler(u_char *userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    */
    ...
    MurmurHash3_x64_128(srcIP, strlen(srcIP), HASH_SEED, srcPortHashValue);
    srcIPHash = srcPortHashValue[0];
    MurmurHash3_x64_128(dstIP, strlen(dstIP), HASH_SEED, dstPortHashValue);
    dstIPHash = dstPortHashValue[0];
